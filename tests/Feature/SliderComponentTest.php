...
it('supports min max step and size', function () {
    $html = Blade::render(<<<'BLADE'
        <x-std::slider name="level" :min="10" :max="50" :step="5" :value="25" size="sm" />
    BLADE);
...
it('marks the control invalid when the invalid prop is true', function () {
    $html = Blade::render(<<<'BLADE'
        <x-std::slider name="volume" :invalid="true" />
    BLADE);
...
it('disables thumbs when the disabled prop is true', function () {
    $html = Blade::render(<<<'BLADE'
        <x-std::slider name="volume" :disabled="true" />
    BLADE);
...
it('renders full compound structure without shortcut', function () {
    $html = Blade::render(<<<'BLADE'
        <x-std::slider name="volume" :value="30" :shortcut="false">
            <x-std::slider.track>
                <x-std::slider.range />
            </x-std::slider.track>
            <x-std::slider.thumb :index="0" :value="30" />
        </x-std::slider>
    BLADE);
...
it('allows width utilities on the root to override the default w-full', function () {
    $html = Blade::render(<<<'BLADE'
        <x-std::slider name="volume" class="w-auto" />
    BLADE);
...
it('wires the control id onto the first thumb', function () {
    $html = Blade::render(<<<'BLADE'
        <x-std::slider name="volume" slider-id="volume-control" :value="[10, 90]" />
    BLADE);
...
it('clamps the initial value within min and max', function () {
    $html = Blade::render(<<<'BLADE'
        <x-std::slider name="volume" :min="0" :max="100" :value="150" />
    BLADE);
