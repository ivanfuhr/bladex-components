...
if (! function_exists('std_typography_defaults')) {
    function std_typography_defaults(): array
    {
        return [
            'scale' => [
...
if (! function_exists('std_merge_typography_config')) {
    function std_merge_typography_config(array $base, array $override): array
    {
...
if (! function_exists('std_typography_config')) {
    function std_typography_config(): array
    {
        $config = std_typography_defaults();

        $vendorTypography = config('std-components.typography', []);

        if (is_array($vendorTypography) && $vendorTypography !== []) {
            $config = std_merge_typography_config($config, $vendorTypography);
        }

        return $config;
    }

    function std_typography_scale(): array
    {
        $defaults = [
            'sm' => ['text' => 'text-sm', 'leading' => 'leading-5'],
            'default' => ['text' => 'text-base', 'leading' => 'leading-6'],
        ...
        $scale = std_typography_config()['scale'] ?? [];
        $merged = [];
    ...
    function std_typography_size_classes(?string $size): string
    {
        ...
        $entry = std_typography_scale()[$key];

        return trim("{$entry['text']} {$entry['leading']}");
    }

    function std_typography_step_up(string $size): string
    {
    ...
    function std_typography_step_down(string $size): string
    {
        $order = ['sm', 'default', 'lg', 'xl'];
        $index = array_search($size, $order, true) ?: 1;
    ...
    function std_font_role_class(string $role): string
    {
        $roles = std_typography_config()['roles'] ?? [];
        $fonts = std_typography_config()['fonts'] ?? [];
    ...
    function std_default_text_size(): string
    {
        $defaults = std_typography_config()['defaults'] ?? [];
        $size = (string) ($defaults['text_size'] ?? 'default');

        return in_array($size, ['sm', 'default', 'lg', 'xl'], true) ? $size : 'default';
    }

    function std_default_heading_level(): int
    {
        $defaults = std_typography_config()['defaults'] ?? [];
    ...
    function std_input_control_classes(?string $inputSize): string
    {
        $scaleSize = $inputSize === 'sm' ? 'sm' : 'default';
        $fileText = std_typography_scale()['sm']['text'];

        return collect([
            std_typography_size_classes($scaleSize),
            std_font_role_class('body'),
            'text-zinc-950 dark:text-zinc-50',
            "file:border-0 file:bg-transparent file:{$fileText} file:font-medium file:text-zinc-950 dark:file:text-zinc-50",
        ])->implode(' ');
    }

    function std_button_label_classes(?string $buttonSize): string
    {
        ...
        return collect([
            $buttonSize === 'xs' ? 'text-xs leading-4' : std_typography_size_classes($scaleSize),
            std_font_role_class('body'),
            'font-medium',
        ])->implode(' ');
    }

    function std_text_variant_classes(?string $variant, bool $forHeading): string
    {
    ...
    function std_text_color_classes(?string $color): string
    {
    ...
    function std_heading_size_for_level(int $level): string
    {
        $normalized = max(1, min(6, $level));
        $anchorLevel = std_default_heading_level();
        $anchorSize = std_typography_step_up(std_default_text_size());
        $offset = $anchorLevel - $normalized;
        $size = $anchorSize;

        if ($offset > 0) {
            for ($i = 0; $i < $offset; $i++) {
                $size = std_typography_step_up($size);
            }
        } elseif ($offset < 0) {
            for ($i = 0; $i < abs($offset); $i++) {
                $size = std_typography_step_down($size);
            }
        }
    ...
    function std_heading_classes(int $level, ?string $variant = null): string
    {
        $weight = $variant === 'subtle' ? 'font-normal' : 'font-semibold';

        return collect([
            'heading',
            std_typography_size_classes(std_heading_size_for_level($level)),
            std_font_role_class('heading'),
            std_text_variant_classes($variant, true),
            $weight,
            'tracking-tight',
        ])->filter()->implode(' ');
    }

    function std_text_classes(?string $size, ?string $variant, ?string $color): string
    {
        return collect([
            'text',
            std_typography_size_classes($size),
            std_font_role_class('body'),
            std_text_variant_classes($variant, false),
            std_text_color_classes($color),
        ])->filter()->implode(' ');
    }

    function std_cursor_classes(string $cursor): string
    {
        return match ($cursor) {
    ...
    function std_interaction_classes(bool $includeReadOnly = true): string
    {
        return collect([
    ...
    function std_is_attribute_active(ComponentAttributeBag $attributes, string $key): bool
    {
    ...
    function std_is_loading(ComponentAttributeBag $attributes): bool
    {
        if (std_is_attribute_active($attributes, 'data-loading')) {
            return true;
        }
    ...
    function std_apply_interaction(
        ...
        if (std_is_loading($attributes)) {
            $attributes = $attributes->merge(['aria-busy' => 'true']);
        }

        $disabled = std_is_attribute_active($attributes, 'disabled')
            || std_is_attribute_active($attributes, 'aria-disabled');
    ...
    function std_field_surface_classes(?string $size, bool $includeReadOnly = true, string $cursor = 'text'): string
    {
        return collect([
            'rounded-md border border-zinc-200 bg-white px-3 py-1 shadow-sm transition-colors',
            std_cursor_classes($cursor),
            std_input_control_classes($size),
            'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950/10 focus-visible:ring-offset-0',
            std_interaction_classes($includeReadOnly),
    ...
    function std_textarea_surface_classes(?string $size, bool $includeReadOnly = true): string
    {
        return collect([
            'rounded-md border border-zinc-200 bg-white px-3 py-2 shadow-sm transition-colors',
            std_cursor_classes('text'),
            std_input_control_classes($size),
            'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950/10 focus-visible:ring-offset-0',
            std_interaction_classes($includeReadOnly),
            'dark:border-zinc-800 dark:bg-zinc-950',
    ...
    function std_label_classes(): string
    {
        return collect([
            std_text_classes('sm', 'strong', null),
            'text-zinc-950 dark:text-zinc-50',
        ])->implode(' ');
    }

    function std_choice_control_classes(string $type = 'checkbox', ?string $size = null): string
    {
    ...
    function std_checkbox_control_classes(?string $size = null): string
    {
        // checked:bg-* must live in scanned Support/helpers sources — View/Components is not @source'd.
        return collect([
            std_choice_control_classes('checkbox', $size),
            'appearance-none',
    ...
    function std_radio_control_classes(?string $size = null): string
    {
        return collect([
            std_choice_control_classes('radio', $size),
            'appearance-none',
    ...
    function std_switch_root_classes(?string $size = null): string
    {
        return collect([
    ...
    function std_switch_track_classes(?string $size = null): string
    {
    ...
    function std_switch_thumb_classes(?string $size = null): string
    {
    ...
    function std_slider_root_classes(?string $size = null): string
    {
        return collect([
    ...
    function std_slider_track_classes(?string $size = null): string
    {
    ...
    function std_slider_range_classes(): string
    {
        return 'slider__range absolute h-full rounded-full bg-zinc-900 dark:bg-zinc-50';
    }

    function std_slider_thumb_classes(?string $size = null): string
    {
        ...
        return collect([
            ...
            'aria-invalid:border-red-500 aria-invalid:ring-red-500/20',
            'dark:aria-invalid:border-red-500',
            std_cursor_classes('pointer'),
        ])->implode(' ');
    }

    function std_invalid_field_classes(): string
    {
        return implode(' ', [
    ...
    function std_select_listbox_classes(?string $size): string
    {
        return collect([
    ...
    function std_select_option_classes(?string $size): string
    {
        return collect([
            'relative flex w-full select-none items-center gap-2 rounded-sm py-1.5 pl-2 pr-8 outline-none',
            std_cursor_classes('pointer'),
            std_text_classes($size === 'sm' ? 'sm' : null, null, null),
            'text-zinc-950 dark:text-zinc-50',
    ...
    function std_normalize_button_variant(?string $variant): string
    {
    ...
    function std_normalize_button_size(?string $size): string
    {
        $size = $size ?? 'default';
    ...
    function std_button_size_classes(
        string $size,
        bool $square,
        bool $iconOnly,
        bool $hasLeading,
    ...
    function std_button_primary_classes(?string $color): string
    {
        $palette = [
    ...
    function std_button_variant_classes(string $variant, ?string $color): string
    {
        return match ($variant) {
            'primary', 'filled' => std_button_primary_classes($color),
            'secondary' => implode(' ', [
    ...
    function std_button_classes(
        ?string $variant = null,
        ?string $size = null,
        ?string $color = null,
        array $options = [],
    ): string {
        $variant = std_normalize_button_variant($variant);
        $size = std_normalize_button_size($size);
        ...
        return collect([
            ...
            $variant !== 'link' ? std_button_label_classes($size) : std_button_label_classes($size).' underline-offset-4',
            'transition-colors',
            ...
            std_interaction_classes(),
            'cursor-pointer aria-disabled:pointer-events-none aria-disabled:cursor-not-allowed',
            '[&_svg]:pointer-events-none [&_svg]:shrink-0',
            $variant !== 'link' ? std_button_size_classes($size, $square, $iconOnly, $hasLeading, $hasTrailing) : null,
            std_button_variant_classes($variant, $color),
        ])->filter()->implode(' ');
    }

    function std_normalize_icon_variant(string $variant): string
    {
        if ($variant === 'solid') {
            return 'outline';
        }
    ...
    function std_icon_variant_resolve(string $variant): array
    {
        $normalized = std_normalize_icon_variant($variant);
    ...
    function std_icon_variant_class_string(string $variant): string
    {
        [$sizeClass] = std_icon_variant_resolve($variant);

        return "block shrink-0 {$sizeClass}";
    }

    function std_normalize_icon_name(string $name): string
    {
        $trimmed = trim($name);
    ...
    function std_resolved_icons_path(): string
    {
        return dirname(__DIR__).'/icons';
    }

    function std_google_fonts_url(): ?string
    {
        $fonts = std_typography_config()['fonts'] ?? [];
    ...
    function std_css_font_variables(): array
    {
        $fonts = std_typography_config()['fonts'] ?? [];
        $variables = [];
    ...
    function std_resolve_timezone(?string $timezone): string
    {
        if (filled($timezone)) {
            return $timezone;
        }
    ...
    function std_to_date_string(mixed $value): string
    {
    ...
    function std_normalize_date_value(mixed $value, string $mode = 'single'): ?string
    {
        if ($value === null || $value === '') {
            return null;
        }
        ...
        if (is_array($value)) {
            if ($mode === 'range') {
                ...
                if (filled($start) && filled($end)) {
                    return std_to_date_string($start).'/'.std_to_date_string($end);
                }

                return null;
            }

            return collect($value)->filter()->map(fn (mixed $v) => std_to_date_string($v))->implode(',');
        }

        return is_string($value) ? $value : std_to_date_string($value);
    }

    function std_normalize_datetime_value(mixed $value, ?string $timezone = null): ?string
    {
        if ($value === null || $value === '') {
            return null;
        }

        $tz = std_resolve_timezone($timezone);
    ...
    function std_normalize_time_value(mixed $value, bool $withSeconds = false): ?string
    {
        if ($value === null || $value === '') {
            return null;
        }
    ...
    function std_date_preset_label(string $key): string
    {
        return match ($key) {
            'today' => __('Today'),
    ...
    function std_date_preset_dates(string $key, ?Carbon $allTimeStart = null): ?array
    {
        $now = Carbon::now();
    ...
    function std_date_preset_metadata(string $keys, ?Carbon $allTimeStart = null): array
    {
        $items = [];
        ...
        foreach (preg_split('/\s+/', trim($keys)) ?: [] as $key) {
            ...
            $dates = std_date_preset_dates($key, $allTimeStart);
            ...
            $items[] = [
                'key' => $key,
                'label' => std_date_preset_label($key),
    ...
    function std_field_has_errors(?string $name): bool
    {
        if (! filled($name)) {
            return false;
        }
...
if (! function_exists('std_merge_described_by')) {
    /**
     * Merge ancestor field description/error ids into control aria-describedby.
     */
    function std_merge_described_by(ComponentAttributeBag $attributes, mixed $describedBy = null): ComponentAttributeBag
    {
        $describedBy ??= std_ancestor_attribute('describedBy');
...
if (! function_exists('std_ancestor_attribute')) {
    /**
     ...
     */
    function std_ancestor_attribute(string $key, mixed $default = null): mixed
    {
